package com.pay.wechatpay.controller.service.impl;
...
import com.pay.wechatpay.controller.service.WeChatPayService;
import com.pay.wechatpay.controller.wxpayutil.WxPayApi;
import com.pay.wechatpay.controller.wxpayutil.WxPayApi.TradeType;
import com.pay.wechatpay.controller.wxpayutil.WxPayApiBaseConfig;
import com.pay.wechatpay.controller.wxpayutil.WxPayApiConfig;
import com.pay.wechatpay.controller.wxpayutil.WxPayApiConfigKit;
import com.wsh.common.exception.BusinessException;
import com.wsh.common.util.BigDecimalUtils;
...
//		// TODO Auto-generated method stub
//		if (StringUtils.isEmptyStr(request.getString("body"))) {
//			request.put("body", Constants.WX_APPPAY_BODY);
//		}
...
//
//		// ValidateHelper.validateNull(request, new
//		// String[]{"accountCode","outTradeNo","totalFee","notifyURL"});
//		// String accountCode = request.getString("accountCode");
...
//		// String notifyUrl =
//		// "http://a8-domain.yitianxian.com:11111/api/wechatpay/wechatPayAsynNotify";
//
...
//		/**
//		 * String accountCode = request.getString("accountCode"); WxPayApiConfig
//		 * wxPayApiConfig = getWxPayApiConfigByCode(accountCode); boolean
//		 * isSandbox = wxPayApiConfig.isSandbox(); String body =
//		 * request.getString("body"); String outTradeNo =
//		 * request.getString("outTradeNo"); String totalFee =
//		 * request.getString("totalFee"); String spbillCreateIp =
//		 * request.getString("spbillCreateIp"); String businessNotifyUrl =
//		 * request.getString("businessNotifyUrl");
//		 * 
//		 * String notifyUrl =
//		 * "http://a8-domain.yitianxian.com:11111/api/wechatpay/wechatPayAsynNotify"
...
//		 * wxPayApiConfig.setSpbillCreateIp
//		 * (StrKit.isBlank(spbillCreateIp)?"127.0.0.1":spbillCreateIp);
//		 * wxPayApiConfig.setNotifyUrl(notifyUrl);
//		 */
...
//				wxPayApiConfig.getPaternerKey()); // 二次签名
...
//				totalFee, buNotifyURL, PayMethodEnum.APP.getCode(), prepay_id,
//				null, timeExpire);
...
//	}
	...
	public ResponseResult<JSONObject> jsAPIPushOrder(JSONObject request) {
		...
		String appId = wxPayApiConfig.getAppId();// "wxafe7f0fac1c27836";
		...
		// 存储支付记录表 TODO start========================================
//		PayRecordDto payRecordDto = buildPayRecord(outTradeNo, accountCode,
//				totalFee, buNotifyURL, PayMethodEnum.JSAPI.getCode(),
//				prepay_id, openId, timeExpire);
//		payRecordService.save(payRecordDto);
		// 存储支付记录表 end========================================
		return response;
	}
...
//
	private WxPayApiConfig buildWxPayApiConfig(JSONObject request) {
		WxPayApiConfig wxPayApiConfig = null;  //要修改
		String body = (StringUtils.isEmptyStr(request.getString("body")) ? Constants.WX_APPPAY_BODY
				: request.getString("body"));
		String outTradeNo = request.getString("outTradeNo");
		String totalFee = BigDecimalUtils.roundBigDecimal(
				BigDecimalUtils.mul(BigDecimalUtils
						.transNullStringToZERO(request.getString("totalFee")),
						BigDecimalUtils.BigDecimal_100), 0).toString();
		logger.info("totalFee===" + totalFee);
		String spbillCreateIp = request.getString("spbillCreateIp");
//		String notifyUrl = payConfiguration.getWxNotifyUrl();
		/** TODO*/
		...
		if (StringUtils.isNotEmpty(expireTime)) {
			time_expire = DateUtils.formatDate(expireTime,
		...
		if (time_expire != null) {
			String timeStart = DateUtils.formatString(now,
					DateUtils.DATE_FORMAT_YYYYMMDDHHMMSS);
			String timeExpire = DateUtils.formatString(time_expire,
					DateUtils.DATE_FORMAT_YYYYMMDDHHMMSS);
		...
		wxPayApiConfig.setTotalFee(totalFee);
		wxPayApiConfig
				.setSpbillCreateIp(StringUtils.isEmptyStr(spbillCreateIp) ? Constants.WX_DEFAULT_IP
						: spbillCreateIp);
		wxPayApiConfig.setNotifyUrl(notifyUrl);
...
//			Date now = new Date();
//			if (StringUtils.isEmptyStr(transactionId)) {
